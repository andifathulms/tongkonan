 ...
 * rule 4 says there is no runtime network — a tile server would put this map
 * one outage away from being a blank rectangle, and would tell somebody else
 * which houses are being looked at.
 *
 * The land is drawn in the same projection the markers are placed by, from the
 * same coordinates the solar arithmetic runs on, so the map cannot disagree
 * with the model — the same argument the elevation shelf makes. Equirectangular
 * and stated as such: at this latitude it stretches nothing anybody would
 * notice, and it keeps a degree of longitude the same width everywhere, which
 * is what makes the graticule readable as a graticule.
 *
...
function SiteMap({ locale }: { locale: Locale }) {
   ...
   */
  const LON_MIN = FRAME.west
  const LON_MAX = FRAME.east
  const LAT_MIN = FRAME.south
  const LAT_MAX = FRAME.north
  const S = 10
