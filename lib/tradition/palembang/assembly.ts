/** The rumah limas, assembled. Pure and deterministic, like the other eight. */
...
import type { Bounds, House, Layout, PalembangKinds, Part, Rules } from './types'
import { buildFrame, resolveLayout } from './frame'
import { buildRoofFrame, buildSimbar, buildTiles } from './roof'
import { PACK, dimsForLayout, normaliseRules } from './rules'

export { SEQUENCE_SECONDS, placedAt, progressAt } from '@/lib/core/assembly'

export type Timeline = CoreTimeline<PalembangKinds>
export type TimelineEntry = CoreTimelineEntry<PalembangKinds>
export type StageSpan = CoreStageSpan<PalembangKinds>
...
export function buildHouse(rules: Rules): BuildResult {
  ...
  const parts = [...frame.parts, ...roof.parts, ...buildTiles(layout), ...buildSimbar(layout)]
