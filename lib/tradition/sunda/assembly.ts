/**
 * The imah, assembled — and the only one here whose part list starts with the
 * ground it stands on.
 */
...
import type { Bounds, House, Layout, SundaKinds, Part, Rules } from './types'
import { buildFrame, resolveLayout } from './frame'
import { buildHateup, buildRoof } from './roof'
import { PACK, dimsForLayout, normaliseRules } from './rules'

export { SEQUENCE_SECONDS, placedAt, progressAt } from '@/lib/core/assembly'

export type Timeline = CoreTimeline<SundaKinds>
export type TimelineEntry = CoreTimelineEntry<SundaKinds>
export type StageSpan = CoreStageSpan<SundaKinds>
...
export function buildHouse(rules: Rules): BuildResult {
  ...
  const parts = [...frame.parts, ...roof.parts, ...buildHateup(layout)]
