...
export const DIMS = {
  ...
  coneBelly: dim(0.10, 'ratio', 'interpolated', 'none', 'Seberapa jauh sisi kerucut menggembung ke luar dari garis lurus antara tanah dan puncak. Nilai sebelumnya, 0,055, hampir tak terlihat dan bentuknya terbaca sebagai kerucut geometris; ijuk yang bertumpuk lapis demi lapis membuat sisinya jelas melengkung. Bacaan penulis atas foto, bukan ukuran dari siapa pun.', 'How far the cone’s side bellies outward from a straight line between the ground and the apex. The previous figure, 0.055, was barely visible and the form read as a geometric cone; thatch laid course on course bows the side out noticeably. The author’s reading of photographs, not anyone’s measurement.'),
  coneShoulder: dim(0.94, 'ratio', 'interpolated', 'none', 'Seberapa tumpul bahu kerucut di dekat puncak. Nilai satu memberi kerucut lurus yang meruncing ke satu titik matematis; nilai kurang dari satu melebarkan bagian atasnya. Ditahan mendekati satu: nilai yang lebih rendah menggemukkan seluruh separuh atas dan bentuknya terbaca sebagai lonceng, bukan kerucut.', 'How blunt the cone’s shoulder is near the top. A value of one gives a straight cone tapering to a mathematical point; less than one widens the upper part. Kept close to one: lower values fatten the whole upper half and the form reads as a bell rather than a cone.'),
  bellyPeak: dim(0.3, 'ratio', 'interpolated', 'none', 'Di mana gembungnya paling besar, sebagai bagian dari tinggi kerucut. Bukan di tengah: lapis ijuk di bawah lebih panjang dan lebih banyak, jadi bahannya menumpuk paling tebal di sepertiga bawah dan bagian atas tetap hampir lurus. Menaruhnya di tengah membuat pinggang bangunan menjadi bagian tergemuknya, dan siluetnya terbaca sebagai lonceng.', 'Where the swell is greatest, as a share of the cone’s height. Not the middle: the courses low down are longer and there are more of them, so the material builds thickest in the lower third and the upper part stays nearly straight. Putting it at mid-height makes the building’s waist its fattest part and the silhouette reads as a bell.'),
  bellySpread: dim(4, 'ratio', 'interpolated', 'none', 'Seberapa terpusat gembung itu di sekitar puncaknya. Nilai besar memusatkannya menjadi tonjolan sempit; nilai kecil menyebarkannya sepanjang kerucut. Harus cukup besar sehingga garis luarnya meninggalkan tanah dengan jari-jari yang mengecil — bila tidak, atap melebar lebih dahulu sebelum menyempit, dan lapis terbawah terdorong ke bawah permukaan tanah.', 'How concentrated the swell is about its peak. Larger values pull it into a narrow bulge; smaller ones spread it along the cone. It has to be large enough that the outline leaves the ground with its radius already shrinking — otherwise the roof widens before it narrows, and the lowest course is pushed below grade.'),
