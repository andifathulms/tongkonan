/**
 * The malige, assembled — a stack whose plan grows at every step, with each
 * storey's brackets placed before the floor they carry.
 */
...
import type { Bounds, House, Layout, Part, Rules, ButonKinds } from './types'
import { buildMalige, resolveLayout } from './frame'
import { PACK, dimsForLayout, normaliseRules } from './rules'

export { SEQUENCE_SECONDS, placedAt, progressAt } from '@/lib/core/assembly'

export type Timeline = CoreTimeline<ButonKinds>
export type TimelineEntry = CoreTimelineEntry<ButonKinds>
export type StageSpan = CoreStageSpan<ButonKinds>
...
export function buildHouse(rules: Rules): BuildResult {
  ...
  const tower = buildMalige(layout)
