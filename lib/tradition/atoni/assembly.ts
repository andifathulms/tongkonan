/**
 * The ume kbubu, assembled — posts, dome, the loft before the thatch closes
 * over it, and the fire under it last.
 */
...
import type { Bounds, House, Layout, Part, Rules, AtoniKinds } from './types'
import { buildHouseParts, resolveLayout } from './frame'
import { PACK, dimsForLayout, normaliseRules } from './rules'

export { SEQUENCE_SECONDS, placedAt, progressAt } from '@/lib/core/assembly'

export type Timeline = CoreTimeline<AtoniKinds>
export type TimelineEntry = CoreTimelineEntry<AtoniKinds>
export type StageSpan = CoreStageSpan<AtoniKinds>
...
export function buildHouse(rules: Rules): BuildResult {
  ...
  const tower = buildHouseParts(layout)
