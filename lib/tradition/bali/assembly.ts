/**
 * The bale, assembled. Pure and deterministic, like the other four.
 */
...
import type { Bounds, BaliKinds, House, Layout, Part, Rules } from './types'
import { buildFrame, resolveLayout } from './frame'
import { buildMurda, buildRoofFrame, buildThatch } from './roof'
import { PACK, dimsForLayout, normaliseRules } from './rules'

export { SEQUENCE_SECONDS, placedAt, progressAt } from '@/lib/core/assembly'

export type Timeline = CoreTimeline<BaliKinds>
export type TimelineEntry = CoreTimelineEntry<BaliKinds>
export type StageSpan = CoreStageSpan<BaliKinds>
...
export function buildHouse(rules: Rules): BuildResult {
  ...
  const parts = [...frame.parts, ...roof.parts, ...buildThatch(layout), ...buildMurda(layout)]
