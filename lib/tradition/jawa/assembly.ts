 ...
 * The joglo, assembled.
 *
 * Pure and deterministic, like the other two: the same call runs in the
...
import type { Bounds, House, JawaKinds, Layout, Part, Rules } from './types'
import { buildFrame, resolveLayout } from './frame'
import { buildGenteng, buildPendhapa, buildRoofFrame } from './roof'
import { PACK, dimsForLayout, normaliseRules } from './rules'

export { SEQUENCE_SECONDS, placedAt, progressAt } from '@/lib/core/assembly'

export type Timeline = CoreTimeline<JawaKinds>
export type TimelineEntry = CoreTimelineEntry<JawaKinds>
export type StageSpan = CoreStageSpan<JawaKinds>
...
export function buildHouse(rules: Rules): BuildResult {
  ...
  const parts = [...frame.parts, ...roof.parts, ...buildGenteng(layout), ...buildPendhapa(layout)]
