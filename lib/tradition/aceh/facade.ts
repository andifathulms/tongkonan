  ...
  const readings: readonly Reading[] = [
    {
      ...
      body: t(
        ...
        'This house lies east–west because prayer is toward the west: the room is already turned that way before anybody stands in it. The other nineteen buildings in this project are turned by something you can see from their own yard — a compass rule of their own, a granary across the way, a river, a road, a stone, the root of a tree, the fall of a hillside. This one is turned by a doctrine held by people in other countries too, and it was the first rule in the collection to arrive from outside the archipelago. The Madurese tanean lanjang, added later, puts its langgar at the west end for the same reason — so this is no longer the only one, and what is particular to this house is that the doctrine turns its whole plan rather than placing one small building.',
      ),
