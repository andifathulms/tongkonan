...
export function createMaterials(tradition: TraditionKey, anisotropy: number): MaterialSet {
  ...
  if (tradition === 'toraja') {
  ...
  } else if (tradition === 'sumbawa') {
    set.kayu = timber(3838, 0.12, 0.6, 0.8)
    ...
    // The swept earth floor, which is also what holds the night's heat.
  ...
  } else if (tradition === 'sunda') {
    set.kayu = timber(1717, 0.2, 0.3, 0.88)
    set.bambu = new THREE.MeshStandardMaterial({ map: tex(bambooCanvas()), roughness: 0.72, metalness: 0 })
    set.ijuk = new THREE.MeshStandardMaterial({
      map: tex(ijukCanvas()),
      roughness: 0.95,
  ...
  } else if (tradition === 'minahasa') {
    set.kayu = timber(1515, 0.35, 0.7, 0.8)
    ...
    // the same leaf — which is why the key is in the shared list.
    set.rumbia = new THREE.MeshStandardMaterial({
      map: tex(rumbiaCanvas()),
      roughness: 0.95,
      metalness: 0,
  ...
  } else if (tradition === 'banjar') {
    set.ulin = timber(606, 0, 0.5, 0.84)
    set.sirap = new THREE.MeshStandardMaterial({ map: tex(sirapCanvas()), roughness: 0.88, metalness: 0 })
  } else if (tradition === 'dani') {
    set.kayu = timber(1212, 0.15, 0.5, 0.88)
    set.alang = new THREE.MeshStandardMaterial({
      map: tex(alangCanvas()),
      roughness: 0.96,
      metalness: 0,
  ...
  } else if (tradition === 'sasak') {
    set.kayu = timber(1111, 0.4, 0.55, 0.8)
    set.alang = new THREE.MeshStandardMaterial({
      map: tex(alangCanvas()),
      roughness: 0.95,
      metalness: 0,
