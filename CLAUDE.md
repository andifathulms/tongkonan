...
**M5 shipped. Ten houses on a tradition-neutral core, all ten on screen.**

- `lib/tradition/toraja/` generates a tongkonan (155 parts, 33 joints), `lib/tradition/minang/` a rumah gadang (263 parts, 62 joints), `lib/tradition/jawa/` a joglo (293 parts, 58 joints), `lib/tradition/manggarai/` a mbaru niang (161 parts, 31 joints), `lib/tradition/bali/` a bale (117 parts, 50 joints), `lib/tradition/nias/` an omo (216 parts, 73 joints), `lib/tradition/dayak/` a rumah betang (189 parts at eight households, and a different number at every other count), `lib/tradition/sumba/` an uma (140 parts, 24 joints), `lib/tradition/palembang/` a rumah limas (264 parts, 105 joints), `lib/tradition/bugis/` a saoraja (116 parts, 86 joints). `lib/core/` holds what is true of any house and knows a word from none of them. `lib/tradition/registry.ts` is the list the app walks. `lib/solar/` is validated against almanac values and serves ten sites — and Bawömataluo is the first north of the equator, which is worth noticing in a project that had quietly assumed one hemisphere.
- `pnpm check` type-checks and runs 502 tests. Every invariant suite runs over at least five rule combinations — eleven structural checks for the tongkonan, sixteen for the rumah gadang, and their own for the joglo, the mbaru niang, the bale, the omo, the betang, the uma, the limas and the saoraja — and every one passes. `checkAgainstSurvey` reports **skipped** in all ten and must stay that way.
...
- **All ten houses have all four routes, in both locales**: `/[locale]/[tradisi]/{bangun,rakit,baca,sumber}`, eighty pages. The four old tradition-less paths still answer and say where the page went, carrying the query with them, because a citable address that stops resolving is not much of a citation.
